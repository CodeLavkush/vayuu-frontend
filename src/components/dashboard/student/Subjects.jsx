...
function Subjects() {
  ...
  const studentState = useSelector((state)=> state.students.data)
  const subjectsState = useSelector((state)=> state.subjects.data)
  const [subjects, setSubjects] = useState([])

  useEffect(()=>{
    const student = studentState.filter((st)=> st.user_id === authUser.id)
    const subjectsData = subjectsState.filter((s)=> s.course_id === student[0]?.course_id)
